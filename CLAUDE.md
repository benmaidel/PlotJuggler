...
- `pj_scene3D/`: 3D scene widget family (robotics viz): TF, pointclouds, occupancy grids, URDF/mesh, markers. Independent 3D logic in `core/`, OpenGL widgets in `widgets/`, tests in `tests/`, standalone dev demos in `demos/` (built whenever the `pj_scene3d_widgets` target exists, i.e. on a default build), and non-compiled dev scripts in `tools/` (the headless screenshot harness). Do not add 3D rendering code elsewhere.
...
- **C++20**.
- **Linux** is the v1 release target. The code **must stay portable** — no Linux-only APIs or POSIX-specific paths in module code; gate anything platform-specific behind the usual CMake / `#ifdef` guards so a macOS/Windows build is a build-system problem, not a code problem. **macOS is supported as a developer build from the build tree** (`./install_qt6.sh` + `./build.sh` + `./run.sh` are macOS-aware via `platform_env.sh`); the app builds, launches, and runs (plotting, playback, data sources, processors, marketplace), and `-Werror` is fatal there exactly as on Linux. **The 2D scene renders** — `MediaViewerWidget` uses Metal on Apple and its shaders are baked by a `qsb` rule covering GLSL 400/410/440 plus MSL. **The 3D scene does not yet**: `SceneViewWidget` needs OpenGL 4.5 + compute (Apple caps at 4.1) and shows a placeholder. A QRhi/Metal renderer for it exists and is tested on both Metal and OpenGL, but is reachable only via the `PJ_SCENE3D_RHI` developer preview; wiring it into the real dock is gated on closing two parity gaps (mesh shadows, the GPU AABB reducer) so macOS does not ship a visibly weaker renderer than Linux. The strategy is dual-backend — OpenGL stays the Linux path — see [`pj_scene3D/docs/ARCHITECTURE.md`](./pj_scene3D/docs/ARCHITECTURE.md) → "Backend strategy" and [`docs/QT_NOTES.md`](./docs/QT_NOTES.md) → "Building on macOS". No macOS packaging (`.app`/`.dmg`) yet.
