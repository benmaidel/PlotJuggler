...
- **C++20**.
- **Linux** is the v1 release target. The code **must stay portable** — no Linux-only APIs or POSIX-specific paths in module code; gate anything platform-specific behind the usual CMake / `#ifdef` guards so a macOS/Windows build is a build-system problem, not a code problem. **macOS is supported as a developer build from the build tree** (`./install_qt6.sh` + `./build.sh` + `./run.sh` are macOS-aware via `platform_env.sh`); the app builds, launches, and runs (plotting, playback, data sources, processors, marketplace), but **neither scene view renders on macOS yet**: `pj_scene3D` needs OpenGL 4.5 + compute (Apple caps at 4.1, so it shows a placeholder), and `pj_scene2D`'s image/video pipeline forces the QRhi OpenGL backend with GLSL-440-only baked shaders that a 4.1 context cannot satisfy. Both are fixed by the same work — a real `qsb` shader-baking rule plus letting QRhi use the platform default (Metal) backend. See [`docs/QT_NOTES.md`](./docs/QT_NOTES.md) → "Building on macOS". No macOS packaging (`.app`/`.dmg`) yet.
