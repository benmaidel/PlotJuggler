...
void RhiAxisPass::rebuildInstances() {
  ...
  // scale — which keeps mat3(model) a valid normal matrix (see arrow.vert).
...
void RhiAxisPass::prepare(QRhiResourceUpdateBatch& updates, const RhiFrameContext& ctx) {
  ...
  if (instances_dirty_) {
    ...
    if (!instances_.empty()) {
      updates.updateDynamicBuffer(
          instance_buf_, 0, static_cast<quint32>(needed * static_cast<int>(sizeof(Instance))), instances_.data());
    }
  ...
  // setFrames() takes world-space transforms, so the shared shader's
  // parent-frame slot is the identity here.
